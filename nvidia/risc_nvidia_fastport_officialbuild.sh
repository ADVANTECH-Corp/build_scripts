...
DAILY_SBOM_VER="${PROJECT}_${OS_VERSION}_v${DAILY_RELEASE_VERSION}_${KERNEL_VERSION}_${TARGET_BOARD}_${SOC_MEM}_${STORAGE}_${DATE}_sbom"
...
OFFICAL_SBOM_VER="${PROJECT}_${OS_VERSION}_v${RELEASE_VERSION}_${KERNEL_VERSION}_${TARGET_BOARD}_${SOC_MEM}_${STORAGE}_${DATE}_sbom"
...
function get_csv_info()
{
	...
	HASH_JETSON_ADV=`cat ${CSV_FILE} | grep "JETSON_ADV" | cut -d ',' -f 2`
	...
	echo "[ADV] HASH_JETSON_ADV : ${HASH_JETSON_ADV}"
    else
...
if [ -z "$EXISTED_VERSION" ] ; then
    ...
    commit_tag adv-jetson-custom $BSP_BRANCH $HASH_JETSON_ADV
