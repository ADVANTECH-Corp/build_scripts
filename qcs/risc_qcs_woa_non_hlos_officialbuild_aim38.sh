...
IMAGE_VER="${MODEL_NAME}${BOARD_VER}${AIM_VERSION}FIV${RELEASE_VERSION}"
VER_TAG="${VER_PREFIX}FBV"$(echo $RELEASE_VERSION | sed 's/[.]//')
...
function get_csv_info()
{
    ...
    CSV_FILE=${CURR_PATH}/${PLATFORM_PREFIX}/${DATE}/${IMAGE_VER}_${DATE}.zip.csv
